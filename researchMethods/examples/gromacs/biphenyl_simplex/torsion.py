...
def main():
    ...
    for popsize in args.num_particles:
        ...
        for attempt in range(args.runs):
            ...
            P = simplex.Simplex(pes.fitness, x0=np.zeros(ndim), maxiter=10e+7, maxfev=10e+10, adaptive=True)
            # P.ncpu = 4
            P.run(pes.fitness, enum_particles=True, add_step_num=True, DEBUG=f'surface_{popsize}.dat')
            # P.run(pes.fitness, enum_particles=True, add_step_num=True, DEBUG='surface.dat')
        ...
        with open(f'results_{popsize}.dat', "a") as res:
            print("\n\n", file=res)
    with open(f"dump_{popsize}.txt", "w") as dump:
